...
async def logUserBot():
    load_dotenv()
    api_id = int(20640363)
    api_hash = "f5e4a99dba7c4477090c60fee6a82e86"
    phone_number = "51929267606"
    ...
    await client.send_message("@spam3232zx", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-1002170875933")
    ...
    while True:
        ...
        try:
            await client.send_message("@spam3232zx", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam 2024","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spam3232zx", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spam3232zx", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(10)
                        if j==2: break
            await client.send_message("@spam3232zx", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
